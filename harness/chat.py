...
def c08(app):
    ...
    got = {d["name"]: str(d["value"]) for d in app.chat.parse_settings(
        "допуск трудоёмкости 10 %, разреши дефицит, не больше трёх договоров оклада")}
    expect(got.get("допуск трудоёмкости") == "10", "допуск: %s" % got.get("допуск трудоёмкости"), out)
    ...
    app.say("допуск трудоёмкости 10 %", model(reply="Записал допуск."))
    expect(str(app.plan_settings()["настройки"].get("допуск трудоёмкости")).startswith("0.1"),
