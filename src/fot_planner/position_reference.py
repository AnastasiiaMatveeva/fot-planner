...
должность, страница положения, номер группы, номер уровня, оклад за 1 ставку.
"""
...
def salary_equivalence_group(
    ...
    страница + номер группы + номер уровня. Поэтому «инженер» и «ведущий инженер»
    оказываются в разных группах, а «инженер» и «программист» — в одной.
    """

    parts: list[str] = []
    if salary_page:
        parts.append(str(salary_page).strip())
