...
def employee_matches_position_spec(
    ...
    *,
    ...
    3. Правила замещения: сотруднику можно дать ставку по соседней должности
    ...
    Окладная группа (страница, номер группы, уровень) в решении больше не
    участвует. Она симметрична — пускала «инженера» и «программиста» друг к
    другу в обе стороны, — и её группировка из положения об оплате труда
    устарела. Взаимозаменяемость теперь задаётся только правилами замещения,
    где она направленная и её ведёт организация. Само поле остаётся: по нему
    строятся графы отчётов.
    """
